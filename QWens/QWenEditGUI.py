...
class QWenEditGUI(QMainWindow):
    def __init__(self,
                 model_path=f"Qwen/Qwen-Image-Edit/",
        ...
        self.negative_prompt_editor = FocusTextEdit("negative prompt")
        # self.prompt_editor.setFocusPolicy(Qt.NoFocus)
        right_panel.addWidget(self.prompt_editor)
        right_panel.addWidget(self.negative_prompt_editor)
    ...
    def run(self):
        self.prompt_editor.setFocusPolicy(Qt.NoFocus)
        self.negative_prompt_editor.setFocusPolicy(Qt.NoFocus)
...
if __name__ == "__main__":
    ...
    window = QWenEditGUI(
        ...
        device="cuda:0")
    peak_mem = torch.cuda.max_memory_allocated()  # 峰值显存
    delta_mem = peak_mem - initial_mem  # 推理增量显存
    print(
        f"initial_mem {initial_mem / 1024 ** 2}(MB)\n",
        f"peak_mem {peak_mem / 1024 ** 2}(MB)\n",
        f"load pipeline consume: {delta_mem / 1024 ** 2}(MB)",
    )
    window.show()
